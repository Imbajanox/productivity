...
(function() {
    ...
    function initSortable() {
        ...
        Object.values(elements.kanbanColumns).forEach(container => {
            ...
            container._sortable = new Sortable(container, {
                ...
                filter: '.empty-column',
                onAdd: function(evt) {
                    // Remove empty-column placeholder when todo is added
                    const emptyCol = evt.to.querySelector('.empty-column');
    ...
    async function handleDragEnd(evt) {
        ...
        // Add empty-column placeholder if source column is now empty
        const fromContainer = evt.from;
        if (fromContainer && fromContainer.querySelectorAll('.todo-card').length === 0) {
            if (!fromContainer.querySelector('.empty-column')) {
                fromContainer.innerHTML = '<div class="empty-column">Keine Todos</div>';
            }
        }
